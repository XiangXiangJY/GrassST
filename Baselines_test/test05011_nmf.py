...
def make_run_id(params):
    return (
        f"nmf_{params['dataset_name']}_"
        f"dim{params['nmf_dim']}_"
        f"iter{params['nmf_max_iter']}_"
...
def split_back_to_slices(adata_all, section_ids, batch_key="section"):
    ...
    for sid in section_ids:
        mask = (adata_all.obs[batch_key] == sid).values
        adata_i = adata_all[mask].copy()
        adatas_split.append(adata_i)
    return adatas_split


def run_joint_nmf_integration(
    adata_all,
    rep_key="X_nmf",
    nmf_dim=30,
    init="nndsvda",
    ...
    if x_min < 0:
        raise ValueError(
            f"NMF requires nonnegative input, but min(X) = {x_min:.6f}. "
            "Please use raw counts or log1p nonnegative matrix instead."
        )

    model = NMF(
        n_components=nmf_dim,
        init=init,
        solver=solver,
        beta_loss=beta_loss,
        max_iter=max_iter,
    ...
    adata_all.obsm[rep_key] = np.asarray(W, dtype=float)
...
def run_rep_spectral_fixed(
    ...
    for adata_i, sid in zip(out_adatas, section_ids):
        ...
        affinity = adata_i.obsp["connectivities"].copy()
...
def concat_method_slices(adatas, section_ids, batch_key="section", rep_key="X_nmf"):
    adata_all = ad.concat(
        adatas,
        label=batch_key,
        keys=section_ids,
...
def main():
    ...
    print(f"[NMF] loaded {len(adatas)} slices")
    ...
    print("\n==================== RUN JOINT NMF INTEGRATION ====================")
    nmf_result = run_joint_nmf_integration(
        adata_all=adata_all,
        rep_key=params["rep_key"],
        nmf_dim=params["nmf_dim"],
        init=params["nmf_init"],
        solver=params["nmf_solver"],
        beta_loss=params["nmf_beta_loss"],
        max_iter=params["nmf_max_iter"],
        random_state=params["random_state"],
    )

    print("NMF embedding shape:", adata_all.obsm[params["rep_key"]].shape)
    print("NMF components shape:", nmf_result["components"].shape)
    print("NMF reconstruction error:", f"{nmf_result['reconstruction_err']:.6f}")
    print("NMF iterations used:", nmf_result["n_iter"])
    print("===============================================================\n")

    np.save(os.path.join(save_root, f"{params['rep_key']}.npy"), adata_all.obsm[params["rep_key"]])
    np.save(os.path.join(save_root, "nmf_components.npy"), nmf_result["components"])

    with open(os.path.join(save_root, "nmf_fit_info.json"), "w") as f:
        json.dump(
            {
                "reconstruction_err": float(nmf_result["reconstruction_err"]),
                "n_iter": int(nmf_result["n_iter"]),
            },
            f,
            indent=2,
        )
    ...
    if "nmf_leiden" in methods_to_run:
        print("\n==================== RUN NMF LEIDEN ====================")
        ...
        method_dir = os.path.join(save_root, "nmf_leiden")
        os.makedirs(method_dir, exist_ok=True)

        for row in rows_leiden:
            print(
                f"nmf_leiden | {row['section_id']} | "
            ...
            metric_rows.append(
                {
                    ...
                    **row,
                    **params,
                    "nmf_reconstruction_err": nmf_result["reconstruction_err"],
                    "nmf_n_iter": nmf_result["n_iter"],
                }
            )
        ...
        print(f"\nnmf_leiden | F1_LISI = {f1_score:.4f}")
    ...
    if "nmf_spectral" in methods_to_run:
        print("\n==================== RUN NMF SPECTRAL ====================")
        ...
        method_dir = os.path.join(save_root, "nmf_spectral")
        os.makedirs(method_dir, exist_ok=True)

        for row in rows_spectral:
            print(
                f"nmf_spectral | {row['section_id']} | "
        ...
        print(f"\nnmf_spectral | F1_LISI = {f1_score:.4f}")
    ...
    global_metrics_path = os.path.join("./results", "metrics", "all_nmf_runs.csv")
    ...
    print("  X_nmf.npy")
    print("  X_umap.npy")
    print("  nmf_components.npy")
    print("  nmf_fit_info.json")
    print("  nmf_leiden/adata_all_plot_ready.h5ad")
    print("  nmf_spectral/adata_all_plot_ready.h5ad")
    print("All done.")
